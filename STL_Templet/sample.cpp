...
namespace jj04
{
	void test_forward_list(long &value)
	{
		...
		for (long i = 0; i < value; ++i)
		{
			...
			{
				snprintf(buf, 10, "%d", rand());
				c.push_front(string(buf)); // no push_back
			}
			catch (exception &p)
			{
				cout << "i=" << i << " " << p.what() << endl;
...
namespace jj08
{
	void test_unordered_multiset(long &value)
	{
		...
		cout << "unordered_multiset.bucket_count()= " << c.bucket_count() << endl; // bucaket 数>元素个数
