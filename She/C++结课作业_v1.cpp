...
inline ostream& operator<<(ostream& os, const Book& b) {
	os << "书籍名称：" << setiosflags(ios_base::left) << setw(56) << b.name << endl
	   << "书籍编号：" << setw(56) << b.num << endl
	   << "书籍作者：" << setw(56) << b.auther << endl;
	if (b.borrow_flag)
		os << "书籍被借出。                                                      \n"
		   << "读者姓名：" << setw(56) << b.reader << endl
		   << "借书证号：" << setw(56) << b.lcn << endl
		   << "借书日期：" << setw(56) << b.data << endl;
	else
		os << "书籍未被借出。                                                    \n";
	return os;
}

inline istream& operator>>(istream& is, Book& b) {
	int lcn;
	string reader;
	string data;
	string name;
	string num;
	string auther;
	is >> reader >> lcn >> data >> name >> num >> auther;
	b.borrow_flag = true;
	b.reader.assign(reader);
	b.lcn = lcn;
	b.data.assign(data);
	b.name.assign(name);
	b.num.assign(num);
	b.auther.assign(auther);
} //not use this
class Library {
public:
	//书籍库
